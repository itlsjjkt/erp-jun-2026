...
class InventoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $month = date('m');
        $year = date('Y');
        $item = DB::table('master_items')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Choose something…', '');
        ...
        if(isAdministrator() || isAdmin() ){
            ...
            ->get()->pluck('name', 'id')->prepend('Choose something…', '');
            ...
            if($cek){
                $location = DB::table('locations')->where('company_id', Auth::user()->company_id)->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Choose something…', '');
                return view('logistic.inventory.index',compact('location','item'));
            } else{
            ...
            if($cek){
                $location = DB::table('locations')->where('id', Auth::user()->location_id)->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Choose something…', '');
                return view('logistic.inventory.index',compact('location','item'));
            } else{
                $location = Auth::user()->location_id;
                return view('logistic.inventory.process',compact('location'));
    ...
    public function datatables()
    {
        ...
        ->select('inventories.*',
        'master_item_products.name AS productName', 'master_item_brands.name AS productBrand','master_item_products.code AS productCode', 'master_item_products.part_number AS productPartNumber',
        ...
        if (isAdministratorCompany()) {
            ...
            $location = Auth::user()->location_id;
            $result->where('inventories.location_id',$location )
            ->where('inventories.is_local', false);
        }elseif(isEmployee()){
            $location = Auth::user()->location_id;
            $result->where('inventories.location_id',$location )
    ...
    public function create()
    {
        ...
        if(isAdministrator()){
            ...
            $location = DB::table('locations')->where('id', Auth::user()->location_id)->orderBy('name','ASC')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Choose something…', '');
        }
    ...
    public function search(Request $request)
    {
        ...
        if($request->input('status')){
            $search .= "<strong> Status: </strong>".$request->input('status');
        ...
        ->select('inventories.*','locations.name AS location','master_item_products.name AS productName', 'master_item_products.code AS productCode', 'master_item_products.part_number AS productPartNumber','master_item_brands.name AS productBrand','measures.name AS unit')
        ->leftJoin('master_item_products', 'master_item_products.id', '=', 'inventories.product_id')
        ->leftJoin('master_item_brands', 'master_item_brands.id', '=', 'master_item_products.brand_id')
        ->leftJoin('locations', 'locations.id', '=', 'inventories.location_id')
        ->leftJoin('measures', 'measures.id', '=', 'master_item_products.measure_id')
        ...
        return view('logistic.inventory.search', compact('data','location', 'search','item'));
    }
        ...
        if( $query->isEmpty() ){
            ...
            return (new FastExcel($query))->download('inventory-'.date('d-m-Y').'.xlsx', function ($inv) {
                return [
                    ...
                    'Part Number'   => $inv->productPartNumber,
    ...
    public function export_aging(Request $request)
    {
        ...
        'master_item_products.name AS productName',
         ...
         'master_item_brands.name AS productBrand','measures.name AS unit',
        'locations.name AS location',
    ...
    public function stock_opname(Request $request)
    {
        ...
        $query = DB::table('inventories')
        ->select('inventories.*','master_item_products.name AS productname', 
        'master_item_products.code AS productcode', 
        'master_item_products.part_number AS productpartnumber',
        'master_item_brands.name AS productbrand','measures.name AS unit',
        'master_items.code AS item_code','master_items.name AS item_name')
        ->leftJoin('master_item_products', 'master_item_products.id', '=', 'inventories.product_id')
        ->leftJoin('master_item_brands', 'master_item_brands.id', '=', 'master_item_products.brand_id')
    ...
    public function history_export(Request $request,$id)
    {
        ...
        $sheet->setCellValue('B8', 'Part Number / Tipe');
    ...
    public function loadData($location_id = null,Request $request)
    {
        ...
        ->orWhere(function ($query) use ($request) {
            $query->where('master_item_products.code', 'ilike','%'.$request->q.'%')
