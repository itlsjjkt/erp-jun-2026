...
class InventoryController extends Controller
{
    ...
    public function before_index()
    {
        ...
        if(isAdministratorCompany()){
            $company = DB::table('companies')->where('id', Auth::user()->company_id)->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih company...', '');
            return view('logistic.inventory.before_index',compact('company'));
        }
        ...
        }elseif(Gate::allows('admin_gudang')){
            $company = DB::table('companies')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih company...', '');
            return view('logistic.inventory.before_index',compact('company'));
        }else{
            $company = DB::table('companies')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih company...', '');
            return view('logistic.inventory.before_index',compact('company'));
   ...
   public function getLocations(Request $request)
    ...
    public function index(Request $request)
    {
        ...
        $year = date('Y');
        $item = DB::table('master_items')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih...', '');
        if(Gate::allows('admin_gudang')){
            if(!$request->location_id){
                $company = DB::table('companies')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih company...', '');
                return view('logistic.inventory.before_index',compact('company'))->with('error','Silahkan cari lokasi terlebih dahulu');
            }
            $cek = InventoryProcess::where('location_id',$request->location_id)
            ->where('month',$month)
            ->where('year',$year)->first();

            if($cek){
                $location = DB::table('locations')->where('id', $request->location_id)->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih...', '');
            ...
            if(!$request->location_id && !Auth::user()->location_id){
                $company = DB::table('companies')->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih company...', '');
                return view('logistic.inventory.before_index',compact('company'))->with('error','Silahkan cari lokasi terlebih dahulu');
            }
            $location = DB::table('locations')->where('id', $request->location_id)->orderBy('name','ASC')->get()->pluck('name', 'id')->prepend('Silahkan pilih...', '');
    ...
    public function datatables(Request $request)
    {
        ...
        $result = DB::table('inventories')
            ...
            ->when($request->filled('location_id'), function($query) use ($request) {
                return $query->where('inventories.location_id', $request->location_id);
            })
            ->when($request->filled('item_id'), function($query) use ($request) {
                return $query->where('master_item_products.item_id', $request->item_id);
            })
            ->when($request->filled('stock_onhand'), function($query) use ($request) {
                return $query->where('inventories.stock_onhand', '>', 0);
            })
            ->when($request->filled('status'), function($query) use ($request) {
                return $query->where('inventories.stock_status', $request->status);
            });

        if (isAdministratorCompany() && !$request->filled('company_id')) {
            $result->where('locations.company_id', Auth::user()->company_id);
        }

        if ((isAdministratorLocation() || isEmployee()) && !$request->filled('location_id')) {
            $result->where('inventories.location_id', Auth::user()->location_id);
        }

        $result->where('inventories.is_local', false);
       ...
       return  DataTables::of($result)
        ...
        ->editColumn('location', function ($result) {
            return $result->location.'-'.$result->companyCode;
        })
        ...
        ->editColumn('productCode', function ($result) {
            if ($result->label_print) {
                // Jika label_print ada, periksa status_label_applied
                if ($result->status_label_applied == 0) {
                    return '<span style="color:blue;">' . $result->productCode . '</span>';
                } else {
                    return '<span style="color:green; font-weight:bold;">' . $result->productCode . '</span>';
                }
            } else {
                // Jika label_print tidak ada, tampilkan produkCode biasa
                return '<span>' . $result->productCode . '</span>';
            }
        })

        ->rawColumns(['action', 'status','productName','kind','price','productCode'])
        ->make(true);
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'image' => 'image|mimes:jpeg,png,jpg,gif|max:5120', // 5120 KB = 5 MB
        ]);
        ...
        if ($request->has('image')) {
            ...
            $name = Str::random(15).time();
            $folder = '/uploads/image_item_inventory/';
    ...
    public function search(Request $request)
    {
        ...
        $query = 'status='.$request->get('status').'&item_id='. $request->get('item_id')."&location_id=".$request->get('location_id')."&stock_onhand=".$request->get('stock_onhand');
        $data = $request->all();
    ...
    public function export(Request $request)
    {
        ...
        if( $query->isEmpty() ){
            ...
            return (new FastExcel($query))->download('inventory-'.date('d-m-Y').'.xlsx', function ($inv) {
                $label_print = $inv->label_print;
                $status_label_applied = $inv->status_label_applied;

                $statusQR = '';
                if($label_print) {
                    if($status_label_applied === 1) {
                        $statusQR = 'Sudah di print dan sudah di tempel';
                    } else {
                        $statusQR = 'Sudah di print';
                    }
                } else {
                    $statusQR = 'Belum di print';
                }
                return [
                    'No'            => $inv->number,
    ...
    public function proses(Request $request)
    {
        ...
        try {
            ...
            // Jalankan perpindahan saldo
            InventoryProcess::process($locationId);

            DB::commit();

            // Ambil company sesuai user
            if (isAdministratorCompany() || isAdministratorLocation()) {
                $company = DB::table('companies')
                            ->where('id', Auth::user()->company_id)
                            ->orderBy('name','ASC')
                            ->pluck('name', 'id')
                            ->prepend('Silahkan pilih company...', '');
            } else {
                $company = DB::table('companies')
                            ->orderBy('name','ASC')
                            ->pluck('name', 'id')
            ...
            // Redirect ke before_index dengan flash message
            return redirect()->route('logistic.inventory.before_index')
                            ->with([
                                'success' => 'Perpindahan Saldo dari Bulan '. $last_month.' ke '.$cur_month.' berhasil',
                                'company' => $company
                            ]);
    ...
    public function adjustment_merge(Request $request)
    {
        if (!Gate::allows('adjustment')) {
            return abort(401);
        }
        if ($request->has('inv_id')) {
            ...
            return redirect()->back()->withErrors(['Belum melakukan Checklist Item Inventory']);
        }
        return view('logistic.inventory.adjustment_merge', compact('inventories'));
    }

    public function storeAdjustment_merge(Request $request)
    {
        if (! Gate::allows('adjustment')) {
            return abort(401);
        }
        $inventCount = count($request->get('inventory_id'));
        for ($i = $inventCount - 1; $i >= 0; $i--){
            $data = [];

            $increment = DB::table('inventory_adjustments')
                ->leftJoin('inventories', 'inventories.id', '=', 'inventory_adjustments.inventory_id')
                ->whereYear("inventory_adjustments.created_at", date('Y'))
                ->where('inventories.location_id', $request->get('location_id')[$i])
                ->get();

            $code = DB::table('locations')
                ->select('locations.alias AS locationCode', 'companies.alias AS companyCode')
                ->leftJoin('companies', 'companies.id', '=', 'locations.company_id')
                ->where('locations.id', $request->get('location_id')[$i])
                ->first();

            $num = sprintf("%'.05d", count($increment) + 1);
            $no = "ADJ-" . $code->companyCode . "-" . $code->locationCode . "-" . date('my') . "-" . $num;

            if($request->get('operator_all') == 1){
                $operatorrr = $request->get('operatorAll');
            }else{
                $operatorrr = $request->get('operator')[$i];
            }
            if($request->get('reason_all') == 1){
                $reasonnn = $request->get('reasonAll')?$request->get('reasonAll'):'-';
            }else{
                $reasonnn = $request->get('reason')[$i]?$request->get('reason')[$i]:'-';
            }
            
            $data['inventory_id'] = $request->get('inventory_id')[$i];
            $data['reason'] = $reasonnn;
            $data['doc_no'] = $no;
            $data['qty_awal'] = $request->get('qty_awal')[$i];
            $data['operator'] = $operatorrr;
            $data['qty_fisik'] = $request->get('qty_fisik')[$i];
            $data['created_by'] = Auth::user()->id;
            ...
            try {
                $inventory = Inventory::findOrFail($request->get('inventory_id')[$i]);
                
                $adjustment = InventoryAdjustment::insert($data);
                $dataInv['stock_onhand'] = $request->get('qty_fisik')[$i];
                if ($request->get('qty_fisik')[$i] > $request->get('qty_awal')[$i]) {
                    $dataInv['in'] = $inventory->in + $request->get('qty_fisik')[$i] - $request->get('qty_awal')[$i];
                } else {
                    $dataInv['out'] = $inventory->out + $request->get('qty_awal')[$i] - $request->get('qty_fisik')[$i];
                }
                $dataInv['stock_status'] = getStatusInventory($inventory->stock_max, $inventory->stock_min, $request->get('qty_fisik')[$i], 'raw');
                $inventory->update($dataInv);

                if ($request->get('qty_fisik')[$i] > $request->get('qty_awal')[$i]) {
                    $invHistory = [
                        'inventory_id' => $request->get('inventory_id')[$i],
                        'qty_in' => $request->get('qty_fisik')[$i] - $request->get('qty_awal')[$i],
                        'qty_awal' => $request->get('qty_awal')[$i],
                        'message' => $no,
                        'description' => $request->get('reason')[$i]
                    ];
                } else {
                    $invHistory = [
                        'inventory_id' => $request->get('inventory_id')[$i],
                        'qty_out' => $request->get('qty_awal')[$i] - $request->get('qty_fisik')[$i],
                        'qty_awal' => $request->get('qty_awal')[$i],
                        'message' => $no,
                        'description' => $request->get('reason')[$i]
                    ];
        ...
        return redirect()->route('logistic.adjustment_stock.index', ['id' => Hashids::encode($request->get('inventory_id')[$i + 1])])->with(['success' => 'Berhasil melakukan Adjustment Stock!']);
    }
    ...
    {
        if (Gate::allows('inventory') || Gate::allows('inventory_monitoring') ) {
            $id = Hashids::decode($id);
    ...
    public function print_single(Request $request, $id)
    {
        $id = Hashids::decode($id);
        ...
        $data = DB::table('inventories')
            ->select(
                ...
                'master_item_brands.name AS brand',
                'master_item_products.code AS doc_no',
                'measures.name AS measure',
                'locations.alias AS lokasi_kode',
                'companies.alias AS company_kode'
            )
            ->leftJoin('master_item_products','master_item_products.id','=','inventories.product_id')
            ->leftJoin('locations','locations.id','=','inventories.location_id')
            ->leftJoin('companies','companies.id','=','locations.company_id')
            ->leftJoin('master_item_brands','master_item_brands.id','=','master_item_products.brand_id')
            ->leftJoin('measures', 'measures.id', '=', 'inventories.measure_id')
            ->where('inventories.id', '=', $inv_id)
            ->first();

        $tinggiMM = $request->get('ukuran');
        $ukuranQr = ($tinggiMM-6) * 3.7;
        $qrCodeData = QrCode::size($ukuranQr)->generate($data->uuid);
        ...
        DB::table('inventories')
            ->where('id', $inv_id)
            ->update(['label_print' => now()]);

        $pdf = Pdf::loadView('logistic.inventory.print_single', compact('data', 'qrCodeImage', 'tinggiMM'))->setPaper([0, 0, $lebarPt, $tinggiPt], 'portrait');
    ...
    public function print_merge(Request $request)
    {
        ...
        $result = DB::table('inventories')
            ...
            ->orderBy('master_item_products.code','ASC')
            ->get();
        ...
        $pdf = Pdf::loadView('logistic.inventory.print_multiple', compact('result', 'tinggiMM'))->setPaper([0, 0, $lebarPt, $tinggiPt], 'portrait');
        return $pdf->download('['.$tinggiMM.'mm]-QRCODE-ALL.pdf');
    }

    public function getInventoryData($uuid)
    {
    ...
    public function scan_qr(){
        $canPrintQR = Gate::allows('print_qr_inventory');
        return view('logistic.inventory.scan_qr',compact('canPrintQR'));
    }
    public function store_scan_qr(Request $request){
        dd($request);
    }
    public function fetch_produk_by_uuid($uuid)
    {
        $inventory = DB::table('inventories')
            ->select(
                'inventories.*',
        ...
        $invHistory = DB::table('inventory_histories')
            ->select(
                'id',
                'message AS doc_no',
                'qty_in',
        ...
        return response()->json([
            ...
            'product_part_number'   => $inventory->product_part_number,
            'stock_onhand'          => $inventory->stock_onhand,
            'initial'               => $inventory->initial,
            'in'                    => $inventory->in,
            'out'                   => $inventory->out,
            'measure_name'          => $inventory->measure_name,
            'code_rack'             => $inventory->code_rack,
            'location_name'         => $inventory->location_name,
            'company_alias'         => $inventory->company_alias,
            'status_label_applied'  => $inventory->status_label_applied,
            'history_data'          => $invHistory
        ]);
    }

    public function store_status_label_applied(Request $request)
    {
        DB::beginTransaction();
        try {
            DB::table('inventories')
                ...
                ->update([
                    'status_label_applied' => 1,
                    'set_label_applied_by' => Auth::user()->id
                ]);
            DB::commit();
            return back()->with('success', 'Status penempelan label berhasil diubah.');
        } catch (\Exception $e) {
            DB::rollBack();
