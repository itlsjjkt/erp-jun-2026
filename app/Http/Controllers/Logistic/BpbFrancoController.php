...
use App\Exports\BpbFrancoExport;
...
class BpbFrancoController extends Controller
{
    use UploadTrait;
    /**
    ...
    public function index()
    {
        if (Gate::allows('bpb') || Gate::allows('bpb_monitoring')) {
            ...
            return view('logistic.bpb_franco.index',compact('location'));
        }else{
            return abort(401);
        }
    }
    ...
    public function datatables(Request $request)
    {
        if (Gate::allows('bpb') || Gate::allows('bpb_monitoring')) {
            ...
            $result  = Bpb::getDataFranco($data);

            return  DataTables::of($result)
            ->addColumn('action', function ($result) {
                $url_edit = "<a href='".route('logistic.bpb_franco.edit', Hashids::encode($result->id))."' title='".trans('Edit')."' data-toggle='tooltip' class='btn btn-outline'><span class='ti-pencil icon-lg'></span> </a>";
                $url_view = "<a href='".route('logistic.bpb_franco.show', Hashids::encode($result->id))."' title='".trans('app.show_title')."' data-toggle='tooltip' class='btn btn-outline'><span class='ti-eye icon-lg'></span> </a>";
                $url_delete = "<form class='delete' action='".route('logistic.bpb_franco.delete', ['id' => $result->id])."' method='POST'>
                                    ".csrf_field()."
    ...
    public function create(Request $request)
    {
        ...
        $po_items = PurchaseOrder::getProductItemFranco($id['0']);

        return view('logistic.bpb_franco.create', compact('po','po_items'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->get('status')==1) {
            ...
            $nomor = "BPB-FRANCO-".date('my')."-". sprintf("%'.05d", $increment + 1) ;
            $data['status']    = 1;
            $data['publish']   = date('Y-m-d H:i:s');

        }else{
            $nomor = "BPB-FRANCO-".date('my')."-DRAFT";
            $data['status']        = 0;
        }
        ...
        if ($request->hasFile('attachment_file')) {
            ...
            $name = 'BPB-FRANCO'.time();
            $folder = '/uploads/bpb_franco/attachment_bpb_franco/'.date('Y').'/';
        ...
        try {
            ...
            $item = $request->get('po_item_id');

            for($i=0;$i < count($item);$i++) {

                if (in_array($request->get('po_item_id')[$i], $request->get('iscreateBPB'))) {
                    $dataBPB[] = [
                        'bpb_id'        => $bpb->id,
                        'spb_item_id'   => $request->get('po_item_id')[$i],
                    ...
                    if($request->get('qty_po')[$i] == $request->get('qty_bpb')[$i]){
                        $cases[]        = "WHEN id = {$request->get('po_item_id')[$i]} THEN 1";
                        $qty_parsial [] = "WHEN id = {$request->get('po_item_id')[$i]} THEN 0";
                        $parsial[]      = '1';
                    }else{
                        $qtyParsial     = $request->get('qty_po')[$i] - $request->get('qty_bpb')[$i];
            ...
            $bpb_items = BpbItem::insert($dataBPB);

            $po = PurchaseOrder::findOrFail($request->get('po_id'));
            $po->update($dataPO);

            if ($request->get('status')==1) {
                $dataInv =  $dataInvMutation = $invHistory = [];
                for($i=0;$i < count($item);$i++) {
                    if (in_array($request->get('po_item_id')[$i], $request->get('iscreateBPB'))) {

                        $inv  = Inventory::where('location_id',$request->get('locationID'))
                        ...
                        if($inv){
                            ...
                            if($request->get('request_type_item')[$i] == 1 ){
                                ...
                                $wto_stock_onhand   = $wto_inv->stock_onhand - $qtyConversion; //GANTI
            ...
            DB::commit();
            return redirect()->route('logistic.bpb_franco.show',Hashids::encode($bpb->id))->with(['success' => 'Input BPB Berhasil!']);
        } catch (\Exception $e) {
    ...
    public function show($id)
    {
        if (Gate::allows('bpb') || Gate::allows('bpb_monitoring')) {
            ...
            return view('logistic.bpb_franco.show', compact('bpb', 'bpb_items'));
        }else{
            return abort(401);
        }
    }
    ...
    public function edit($id)
    {
        ...
        return view('logistic.bpb_franco.edit', compact('bpb','bpb_items'));
    }
    ...
    public function delete(Request $request)
    {
        ...
        foreach($bpb_item as $val){
            $po_items = PurchaseOrderItem::findOrFail($val->spb_item_id);
            ...
            if($item > 0){
                $qty_parsial = $val->qty + $po_items->qty_parsial;
                $dataPO = array (
                    'lpb_status'  => '2',
        ...
        $po = PurchaseOrder::findOrFail($bpb->po_id);
        $po->update($dataPO);

        $bpb->delete();
        return redirect()->route('logistic.bpb_franco.index')->with(['success' => 'Delete Data Berhasil!']);
        ...
        try {
            ...
            for($i=0;$i < count($item);$i++) {
                ...
                if($inv){
                    ...
                    $dataInvExist['in']           = $inv->in + $request->get('qty')[$i];
            ...
            DB::commit();
            return redirect()->route('logistic.bpb_franco.show',Hashids::encode($bpb->id))->with(['success' => 'Publish Data Berhasil!']);
        } catch (\Exception $e) {
    ...
    public function search(Request $request)
    {
        if (Gate::allows('bpb') || Gate::allows('bpb_monitoring')) {
            ...
            return view('logistic.bpb_franco.search', compact('query', 'search','data'));
        }else{
            return abort(401);
        }
    }
    ...
    public function list()
    {
        ...
        if($cek){
           return view('logistic.bpb_franco.list');
        } else{
    ...
    public function listDatatables()
    {
        ...
        return  DataTables::of($result)
        ->addColumn('action', function ($result) {
            $url_add = "<a href='".route('logistic.bpb_franco.create', ['id' => Hashids::encode($result->id)])."' data-toggle='tooltip' class='btn btn-danger btn-sm font-weight-bold'><span class='ti-file'></span> BUAT LPB </a>";
    ...
    {
        $date = date('Y-m-d');
        return Excel::download(new BpbFrancoExport($request->get('start_date'), $request->get('end_date')), 'Report-BPB-'.$date.'.xlsx');
    }

    public function getDokumenBpbFrancoById($id){
    ...
    public function uploadDokumenBpbFranco(Request $request)
    {
        if (! Gate::allows('bpb')) {
            return abort(401);
        }

        $id = $request->get('bpb_franco_id');
        $bpb_franco = Bpb::findOrFail($id);
        $data = $request->all();
        if ($request->hasFile('attachment_file')) {
            $file = $request->file('attachment_file');
            $name = 'BPB-FRANCO'.time();
        ...
        $bpb_franco->update($data);

        return redirect()->route('logistic.bpb_franco.index')->with('success', 'Update Attachment Dokumen <a href="' . route('logistic.bpb_franco.show', Hashids::encode($bpb_franco->id)) . '" title="' . trans('app.show_title') . '" data-toggle="tooltip">'.$bpb_franco->doc_no.'</a> Berhasil');
    }
