...
class ReturnOutExport implements FromView
{
    ...
    public function view(): View
    {
        ...
        ->select(
            ...
            'inventory_return_out.doc_no',
            'inventory_return_out.created_at',
            'inventory_return_out.operator',
        ...
        ->leftJoin('inventory_return_out', 'inventory_return_out.id', '=', 'inventory_return_items.inventory_return_out_id')
        ...
        ->when(!empty($this->doc_no), function ($query) {
            return $query->where('inventory_return_out.doc_no',$this->doc_no);
        })->when(!empty($this->location_id), function ($query) {
            return $query->where('inventory_return_out.location_id',$this->location_id);
            ...
            if($this->end_date){
                ...
                return $query->whereBetween('inventory_return_out.created_at', [$start , $end]);
            }else{
                return $query->where('inventory_return_out.created_at', $this->start_date);
            }
        })
        /**->where('inventory_return_out.status',1)
        */
        ...
        return view('exports.return_out', [
            'return_out'  => $result ,
        ]);
