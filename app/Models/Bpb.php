...
class Bpb extends Model
{
   ...
   /**
    * Versi aman-publik dari getProductItem (tanpa dependensi Auth::user()).
    * Dipakai oleh halaman verifikasi QR yang diakses tanpa login.
    */
   public static function getProductItemPublic($id){

        return DB::table('bpb_items')
            ->select(
            DB::raw('cast(spb_kolis.no AS INT) as no_koli'),
            'bpb_items.*',
            'spb_kolis.id AS idKoli',
