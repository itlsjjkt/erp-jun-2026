...
class InventoryReturnOut extends Model
{
     ...
     * @var string
     */
    protected $table = 'inventory_return_out';
    /**
     ...
     * @var array
     */
    protected $fillable = ['doc_no','status','created_at','created_by','operator','file','publish','location_id'];

    public function searchableAs()
    {
        return 'inventory_return_out';
    }

    public static function getById($id){
        $query = DB::table('inventory_return_out')
        ->select(
        ...
        ->where('inventory_return_out.id', $id)
        ->first();
    ...
    public static function getData($request, $company = null, $location = null){
        ...
        ->when(!empty($request['location_id']), function ($query) use ($request){
            return $query->where('inventory_return_out.location_id',$request['location_id']);
        }) 
