	...
	<div class="row mB-40">
        ...
        <div class="col-sm-9">
            <div class="bgc-white p-20 bd">
                ...
                <hr class="mB-30">
                {!! Form::open(['method' => 'POST', 'route' => ['purchasing.approval.store_dph'] ]) !!}
                                    ...
                                    @if (count($approval) > 0)
                                        ...
                                        @foreach ($approval as $key)
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="number" name="step[]" class ='form-control' value="<?php echo $key->step;?>" oninput="this.value = Math.abs(this.value)" onwheel="return false;" required>
                                                </td>
                                                <td>
...
<script  type='text/javascript'>
	$(document).ready(function() {
        ...
        $(document).on("click", ".add", function(e) {
            ...
            '</select>' +
            '</td>' +
            '<td><input type="number" class="form-control" name="step[]" oninput="this.value = Math.abs(this.value)" onwheel="return false;" required/></td>' +
            '<td><button class="btn btn-outline text-danger icon-lg remove" ><i class="ti-trash"></i></button></td>'+
            '</tr>');
            $('#user_'+i).select2({
                placeholder: 'Cari Pegawai...',
                ajax: {
                    url: '{{ route("master.get_user_purchasing") }}',
                    dataType: 'json',
                    delay: 250,
