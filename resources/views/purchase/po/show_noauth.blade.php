            ...
            <div class="d-block">
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    @foreach ($result as $docData)
                        <li class="nav-item">
                            <a class="nav-link {{$index==0?'active':''}}" data-toggle="tab" href="#data{{$docData->id}}" role="tab" >{{$docData->doc_no}}</a>
                        </li>
                        @php
                            $index++;
                        @endphp
            ...
            <div class="tab-content mt-5">
                ...
                @foreach ($result as $po)
                    ...
                    if ($po->po_term_id == 1) {
                        ...
                        $t_date      = 'Date';
                        $t_phone  = 'HP';
                        $locationnn = 'Location/Vessel';
                    }
                    ?>

                    <div class="tab-pane {{$index2==0?'active':''}}" id="data{{$po->id}}" role="tabpanel" aria-labelledby="data{{$po->id}}">
                    <div id="watermark" class="watermark">
                        @if($po->status == 6 || $po->status == 8)
                            C A N C E L
                        @else
                            O R I G I N A L
                        @endif
                                            ...
                                            <td class="border-0 text-center" style="width:40%">
                                                ...
                                                <table class="border-0 text-left" style="width:100%">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="border-0">: {{ $po->location }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="border-0">Departement</td>
                                ...
                                <div class="main">
                                    ...
                                    <table class="table table-bordered" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            $no = 1;
                                            $subtotal = 0;
                                            $po_items = getProductItemPo($po->id);
                                            @endphp
                                            ...
                                            <tr>
                                                <td colspan="5" rowspan="9" style="vertical-align:top !important;border-bottom:0 !important;border-top:0 !important">
                                                    <table>
                                                        ...
                                                        <tr>
                                                            <td class="border-0 p-0" style="width: 130px;border:none !important">{{ $t_delivery_date }}</td>
                                            ...
                                            <tr>
                                                <td colspan="5" rowspan="3" style="padding: 0px !important;vertical-align: top;">
                                                    <table>
                                                        <tr>
                                                            <td style="border-bottom:0 !important; border-top:0 !important; border-left:0 !important; width:30%; vertical-align: top !important;">
                                                                {{ $t_confirm }} <br>
                                                                {{ $t_confirm_name }}: <br>
                                                                {{ $t_confirm_date }}: <br>
                                                                {!! $t_confirm_caption !!}
                                            ...
                                            <tr>
                                                <td colspan="2" class="text-center">
                                                    @if($po->approved_by != 0 && in_array($po->status, [2, 4, 5]) || $po->approved_by != null && in_array($po->status, [2, 4, 5]))
                                                        ...
                                                        @endphp
                                                        @if($po->created_by == getApprovalFirstPurchasing($po->id)->user_id)
                ...
                <div style="display: flex; justify-content: center; align-items: center;">
                    <a href="#" data-toggle="collapse" data-target="#history{{$po->id}}" class="btn btn-outline border-dark" style="background-color: #c3e6cb; width:500px;">
                        <i class="ti-time icon-lg"></i> SHOW HISTORY {{$po->doc_no}}
                    </a>
                </div>
                <div class="collapse" id="history{{$po->id}}" aria-expanded="false">
                    <div class="d-flex justify-content-center align-items-center">
                        <div class="tab-pane " id="tab2" role="tabpanel" style="background-color:#0000000c; padding: 20px; max-width: 800px; border-left: 2px solid #000000; border-right: 2px solid #000000; border-bottom: 2px solid #000000; border-top: none; border-top-left-radius: 0; border-top-right-radius: 0; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
                            <div style="border-left: 2px solid #1500b4; padding-left:10px;">
                                ...
                                <div>
                                    @php
                                        $po_history = gethistoryPo($po->id);
                                    @endphp
                                    @foreach ($po_history as $val)
                                        <div style="width: 25px; height: 25px; background-color: #1500b4; border-radius: 50%; position: relative; left: -23px; top:45px;"></div>
                                        <div style="margin-left: 30px;">
                                            ...
                                            @elseif ($val->jenis == 'draft')
                                                @if ($po->dph_id)
                                                    menyetujui pembuatan PO dengan status Draft
                                                @else
                                                    melakukan pembuatan PO dengan status Draft
                                                @endif
                                            @elseif ($val->jenis == 'approval')
                                                melakukan Persetujuan PO
                                                <p><strong>Catatan: </strong>{{ $val->message }}</p>
                                            @elseif ($val->jenis == 'revisi')
                                                melakukan permintaan Perbaikan PO
                                                <p><strong>Catatan: </strong>{{ $val->message }}</p>
                                            @else
                                                melakukan publish PO
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
