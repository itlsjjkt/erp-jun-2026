...
<div class="mB-40">
        ...
        <div class="bgc-white p-30 bd">
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group row">
                        {{-- @if($po->status == 10) --}}
                            ...
                            <div class="col-sm-8">
                                ...
                                <span class="form-control" style="background-color:rgb(230, 230, 230)">
                                    {{getSupplierById($po->supplier_id)}}
                                </span>
                            </div>
                        {{-- @else
                        <label class="col-sm-3">Nama Supplier <span class="text-danger">*</span></label>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <div class="input-group">
                                ...
                                {!! Form::text('due_date_payment', $po->due_date_payment ? date('m/d/Y',strtotime( $po->due_date_payment)) : '', ['class' => 'form-control datepicker due_date_payment', 'placeholder' => '', 'required' => '']) !!}
                            </div>
                        </div>
                        <div class="col-sm-4">
                            {!! Form::select('payment_method', $payment_method, old('payment_method')??'BANK TRANSFER', ['class' => 'form-control select2', 'required' => '']) !!}
                        </div>
                    </div>
                    ...
                    {{-- <div class="form-group row">
                        <label class="col-sm-3">Tgl Pengiriman<span class="text-danger">*</span> <br>& Estimasi Tiba<span class="text-danger">*</span> </label>
                        <div class="col-sm-4">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="ti-calendar"></i></span>
                                </div>
                                {!! Form::text('delivery_date', $po->delivery_date ? date('m/d/Y',strtotime( $po->delivery_date)) : '' , ['class' => 'form-control datepicker delivery_date', 'placeholder' => 'Tanggal Pengiriman','required' =>'']) !!}
                            </div>
                        </div>

                        <span style="font-weight: bold; display: block; margin-top: 8px;">-</span>
                        ...
                        <div class="col-sm-4">
                            <div class="input-group">
                                ...
                                {!! Form::text('estimated_receipt', $po->estimated_receipt ? date('m/d/Y',strtotime( $po->estimated_receipt)) : '' , ['class' => 'form-control datepicker estimated_receipt', 'placeholder' => 'Estimasi Tiba','required' =>'']) !!}
                            </div>
                        </div>
                    </div> --}}
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            ...
                            <span>
                                <input type="checkbox" name="send_expense_ppn" id="send_expense_ppn" class="switch switch-info mt-2" value="{{$po->send_expense_ppn != 0 ? 1 : 0}}"> <label for="send_expense_ppn">PPN Pengiriman</label>
                            </span>
                        </div>
                    </div>
                ...
                <div class="col-sm-6">
					...
					<div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            {!! Form::select('po_note', $po_note, old('po_note')??9, ['class' => 'form-control select2 po_note', 'required' => '']) !!}
            ...
            <div class="row ">
                <div class="col-lg-12">
                    <div class="float-right">
                        <input type="hidden" name="is_item_ppn" id="is_item_ppn" class="magic-checkbox" value="0">
                    ...
                    @if (count($po_items) > 0)
                        ...
                        @foreach ($po_items as $item)
                            ...
                            <tr>
                                ...
                                <td>
                                    <input {{ $po->status == 10 ? 'readonly' : '' }} name="qty_po[]" min="0" class="form-control text-right" value="{{$item->qty }}" id='qty_po_{{$item->id}}' oninput="this.value = Math.abs(this.value)" onkeypress="return event.charCode >= 48">
                                    @if ($item->po_status == '2')
                                    ...
                                    @endif
                                    <input name="qtyAwal[]" type="hidden" value="{{ $item->qty }}">
                                    <input name="qtyparsialawalPR[]" type="hidden" value="{{$item->qty_parsial+$item->qty}}">
                                    <input name="measure_id[]" type="hidden" value="{{ $item->measure }}">
                                    <input name="statusss[]" type="hidden" value="{{$item->po_status}}">
                                </td>
                                <td>{{ $item->measure }}</td>
                                <td><input {{ $po->status == 10 ? 'readonly' : '' }} type='text' name="price[]" class="form-control currency text-right next_price" value="{{ $item->price }}" id="price_{{ $item->id }}"> </td>
                                <td><input {{ $po->status == 10 ? 'readonly' : '' }} type="number" name="diskon_item[]" class="form-control diskon" data-id="{{ $item->id }}" id="discount_{{ $item->id }}" value="{{ $item->discount }}" min="0" step="0.01" max="100" onwheel="return false;"> </td>
                                <td><input type="text" readonly class="form-control text-right totalPrice"  id="totalPrice_{{ $item->id }}" value="{{ $totalPrice }}"> </td>
                                <td style="max-width: 50px;">
                                    @if ($po->status != 10)
                                        <a href="{{ route('purchasing.po.remove_item',['id'=>$item->id ]) }}" title="Hapus Item" data-toggle='tooltip' class='btn btn-outline' id="btn-remove"><span class='ti-trash text-danger icon-lg'></span> </button>
                                    @endif
                                </td>
                            </tr>
                        @php
        ...
        <div class="mt-4">
            ...
            @endif
            @if($po->status==3 || $po->status==9 || $po->status==10)
                <input type="hidden" value="{{$po->status}}" name="status">
...
function calculatePrice(id){
    ...
    function deleteItem(id) {
        ...
        Swal.fire({
            title: "Are you sure?",
            text: "Dengan menghapus Item PO akan dikembalikan ke PR dan halaman akan di refresh, apakah anda yakin?",
            type: "warning",
