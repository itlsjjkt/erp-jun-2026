...
@section('page-header')
    Berita Penerimaan Barang
@stop

@section('breadcrumbs')
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route(ADMIN . '.dashboard') }}">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ route('logistic.bpb.index') }}">Berita Penerimaan Barang</a></li>
...
@section('content')
    {!! Form::open(['method' => 'POST', 'route' => ['logistic.bpb.store'], 'id' => 'formBPB']) !!}
    ...
    <div class="bgc-white p-30 bd">
        ...
        <hr class='mB-30'>
        <h6 class="icon-lg"><strong>{{ $spb->doc_no }}</strong></h6>
        <div class="row mt-5">
            <div class="col-lg-6">
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th class="text-center">Nomor DPM</th>
                    <th class="text-center">Lokasi Inventory</th>
                    <th class="text-center">Nomor PO</th>
                    <th class="text-center">QTY <br> Diterima</th>
            ...
            <tbody>
                ...
                @foreach ($spb_items as $data)
                    <tr>
                        <input name="spb_item_id[]" type="hidden" value="{{ $data->idKoli }}">
                        <input name="location_id[]" type="hidden" value="{{ $data->location_id }}">
                        <input name="pr_item_id[]" type="hidden" value="{{ $data->pr_item_id }}">
                        <input name="no_po[]" type="hidden" value="{{ $data->noPO }}">
                        ...
                        <td class="text-center"><input type="checkbox" name="iscreateBPB[]" class="checkSingle form-control magic-checkbox"  value="{{ $data->idKoli }}" id="checkbox_{{ $data->idKoli }}"><label for="checkbox_{{ $data->idKoli }}"></label></td>
                        ...
                        <td class="text-center">
                            @if ($data->bpb_status == 2)
                                <table class="table border-0">
                                    <tr>
                                        <th style="width:50%">Awal</th>
                                        <th style="width:50%"><span class="text-danger">Belum diterima </span></th>
                                    </tr>
                                    <tr>
                                        <td>{{$data->qtyKoli}} </td>
                                        <td>{{$data->qty_parsial}}</td>
                                    </tr>
                                </table>
                                <input name="qty_spb[]" type="hidden" value="{{$data->qty_parsial}}"  id='qty_spb_{{$data->idKoli}}'>
                            @else
                                {{$data->qtyKoli}}
                                <input name="qty_spb[]" type="hidden" value="{{$data->qtyKoli}}"  id='qty_spb_{{$data->idKoli}}'>
                            @endif
                        </td>
                        <td>{{ $data->measure }}</td>
                        <td>{{ $data->noDPM }}</td>
                        ...
                        <td>
                            @if ($data->bpb_status == 2)
                                <input type='number' name="qty_bpb[]" class="form-control text-center" id='qty_bpb_{{$data->idKoli}}' required  oninput="this.value = Math.abs(this.value)"  value="{{$data->qty_parsial}}">
                            @else
                                <input type='number' name="qty_bpb[]" class="form-control text-center" id='qty_bpb_{{$data->idKoli}}' required  oninput="this.value = Math.abs(this.value)"  value="{{$data->qtyKoli}}">
                            @endif
                        </td>
    ...
    <div class="mt-4">
        <a href="{{ route('logistic.bpb.index') }}" id="btn-cancel" class="btn btn-light text-uppercase fsz-sm fw-600 mr-1">{{ trans('Cancel') }}</a>
...
<script  type='text/javascript'>
	$(document).ready(function() {
        ...
        @foreach ($spb_items as $item)
         $('#qty_bpb_{{ $item->idKoli }}').on('keyup', function(e) {
                var qty_spb  = $('#qty_spb_{{ $item->idKoli }}').val();
                var qty_bpb = $('#qty_bpb_{{ $item->idKoli }}').val();
                if(parseInt(qty_bpb) > parseInt(qty_spb)){
                    e.preventDefault();
                    Swal.fire(
                    'Peringatan!',
