...
<div class="mB-40">
		{!! Form::model($bpb, [
				'action' => ['Logistic\BpbController@update', $bpb->id],
				'method' => 'put',
		...
		!!}
        <input type="hidden" name="spb_id" value="{{ $bpb->spb_id }}">

            <div class="bgc-white p-30 bd">
            <h6><a class="float-left" href="{{ route('logistic.bpb.index') }}"><i class="ti-arrow-left mR-10"></i></a> {{ trans('Back') }}</h6>
            <hr class='mB-30'>
                
            <h6 class="icon-lg"><strong>{{ ($bpb->spb) ? $bpb->spb->doc_no : '' }}</strong></h6>
            <div class="row mt-5">
                <div class="col-lg-6">
					...
					@foreach ($bpb_items as $item)
						<tr>
							...
							<td>
								...
								<input name="qty[]" type="hidden" value="{{ $item->qtyKoli }}" >
							...
							<td class="text-center">
								@if ($item->bpb_status == 2)
									<table class="table border-0">
										...
										<tr>
											<td>{{$item->qtyKoli}} </td>
											<td>{{$item->qty_parsial}}</td>
										</tr>
									</table>
									<input name="qty_spb[]" type="hidden" value="{{$item->qty_parsial}}"  id='qty_spb_{{$item->idKoli}}'>
								@else
									{{$item->qtyKoli}}
									<input name="qty_spb[]" type="hidden" value="{{$item->qtyKoli}}"  id='qty_spb_{{$item->idKoli}}'>
								@endif
							</td>
							<td>{{ $item->measure }}</td>
							<td>{{ $item->noDPM }}</td>
							<td>{{ $item->noPO }}</td>
							<td><input type='number' name="qty_bpb[]" value="{{ $item->qty }}" class="form-control" id='qty_bpb_{{$item->idKoli}}' required min="1" oninput="this.value = Math.abs(this.value)" ></input></td>
        ...
        <div class="mt-4">
            <a href="{{ route('logistic.bpb.index') }}" id="btn-cancel" class="btn btn-light mr-1 text-uppercase fsz-sm fw-600">{{ trans('Cancel') }}</a>
