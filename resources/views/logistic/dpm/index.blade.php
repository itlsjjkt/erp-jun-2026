...
@section('content')
    ...
    <div class="bgc-white bd bdrs-3 p-20 mB-20">
        ...
        <div class="mB-20">
            ...
            <a href="{{ route('purchase_request.list_wti') }}" class="btn btn-primary text-uppercase fsz-sm fw-600">
                Tambah DPM Penggantian
            </a>
            ...
            <form class="form-horizontal" id="form" method='GET'>
                ...
                <div class="row">
                    <div class="col">
                        ...
                        {{-- @if(Auth::user()->id == 231 || Auth::user()->id == 1 || Auth::user()->id == 305) --}}
                        <button type="submit" class="btn btn-warning mt-3 text-uppercase" id="btn-export-new">EXPORT DATA NEW FORMAT</button>
                        {{-- <button type="submit" class="btn btn-info mt-3 text-uppercase" id="btn-sla">EXPORT SLA DATA</button> --}}
                        {{-- @endif --}}
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            company.select2({
                ...
                location.empty();
                $.ajax({
                    url:"{{ route('master.get_location') }}/" + company.val(),
                    type:'GET',
                ...
                $.ajax({
                    url:"{{ route('master.get_department') }}/" + company.val(),
                    type:'GET',
                    success:function(data) {
                        department.empty();
            ...
            $('#btn-export-new').click( function(e) {
                e.preventDefault();
                $('form#form').attr('action',"{{ route('purchase_request.export_new') }}");
                if($('form#form').valid()){
                    $('form#form').submit();
                }
            });

            $('#btn-sla').click( function(e) {
                e.preventDefault();
                $('form#form').attr('action',"{{ route('purchase_request.export_sla') }}");
                if($('form#form').valid()){
                    $('form#form').submit();
                }
            });
