...
<div class="mB-40">
		{!! Form::model($spb, [
				'action' => ['Logistic\SpbController@revision'],
				'method' => 'post', 
        ...
        <input type="hidden" name="spb_id" value="{{ $spb->id }}">
            ...
            <h6 class="font-weight-bold text-bold mb-0">{{ $spb->doc_no }}</h6>
            ...
            Daftar Laporan Penerimaan Barang (LPB) yang akan dibuatkan Surat Pengantar Barang. 
            </div>
                                                    ...
                                                    @foreach ($spb_items as $val)
                                                        <tr>
                                                            ...
                                                            <td style="vertical-align:top !important">
                                                                {{ $val->productCode }} - {{ $val->product }} 
									                            <br>{!! $val->productPartNumber != NULL ? '<small> PN: '.$val->productPartNumber.'</small>' : '' !!} {!! $val->productBrand != NULL ? '<small>Brand: '.$val->productBrand.'</small>' : '' !!}
                                                                <input name="product_id[]" type="hidden" value="{{ $val->id }}">
                                                            </td>
                                                            <td style="vertical-align:top !important">{!! $val->specification !!}</td>
                                                            <td class="text-center" style="vertical-align:top !important" >{{ $val->qtyLpb }} {{ $val->measure }}</td>
                                                            <td>
                                                                <input type="hidden" value="{{ $val->qtyLpb }}" class="form-control" id="qty_lpb_{{ $val->spb_item_id }}" >
                                                                <input type="number" name="qty_spb[]" class="form-control" value="{{ $val->qty }}" required  min="1" id="qty_spb_{{ $val->spb_item_id }}" oninput="this.value = Math.abs(this.value)" onwheel="return false;">  
                                                            </td>
        ...
        <div class="mt-4">
            ...
            <input class="btn btn-success text-uppercase fsz-sm fw-600" type="submit" name="save" value="SUBMIT" id="btn-submit">
        </div>
