...
@section('page-header')
	View Asuransi Unit <small>{{ $insurance->doc_no }}</small>
@stop
...
<div class="mB-40">
	<div class="bgc-white p-30 bd">
		...
		<div class="row mb-1 justify-content-end">
			<div class="col-sm-6">
				<a class="btn btn-outline float-right" href="#" title="Print Data" onclick='printExternal("/logistic/insurance_unit_print/{{ Hashids::encode($insurance->id) }}/print")'><i class="ti-printer icon-lg"></i></a>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-sm-7">: {{ $insurance->kepada }}</div>
		</div>
		<div class="row">
			<label class="col-sm-2">CC</label>
			<div class="col-sm-7">: {{ $insurance->cc }}</div>
		</div>
		<div class="row">
			<label class="col-sm-2">Dari</label>
			<div class="col-sm-7">: Logistik</div>
		</div>
		<div class="row">
			<label class="col-sm-2">Tanggal</label>
			<div class="col-sm-7">: {{ idDate($insurance->created_at) }}
			</div>
		</div>
		<div class="row">
			<label class="col-sm-2">Perihal</label>
			<div class="col-sm-7">: {{ $insurance->perihal }}</div>
		</div>
		...
		<div class="mt-4 btn-group">
			...
			<a href="{{ route('logistic.insurance_unit.edit', Hashids::encode($insurance->id)) }}" class="btn btn-info mr-1 text-uppercase fsz-sm fw-600">Edit Draft</a>
			<form class='delete' action="{{ route('logistic.insurance_unit.delete', ['id' => $insurance->id]) }}" method='POST'>
				{{ csrf_field() }}
