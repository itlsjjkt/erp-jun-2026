...
@section('content')
    {!! Form::model($return_out, [
            'action' => ['Logistic\ReturnOutController@update', $return_out->id],
            'method' => 'put', 
    ...
    !!}
        <div class="bgc-white p-30 bd">
            <h6>Edit ROT {{$return_out->doc_no}}</h6>
            <hr class='mB-30'>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    ...
                    @if ($return_out->file)
                        <code> {{ asset('storage'.$return_out->file) }}</code> 
                    @endif
                </div>
            </div>

            <div class="form-group row">
                <label class="col-sm-2">Location  <span class="text-danger">*</span></label>
                <div class="col-sm-2">
                    <input type="text" readonly class="form-control" value="{{ getDataByID('locations',$return_out->location_id )->name }}">
                    <input type="hidden" name="location_id" id="location_id" value="{{ $return_out->location_id }}">
                    <p class="help-block"></p>
            ...
            <table class="table table-bordered mt-2">
                ...
                <tbody class="item_form" id="itemDPM">
                    @foreach($return_out_items as $item) 
                        <tr class="product_1">
                            <input type="hidden" value="{{ $item->id }}" name="return_out_itemID[]">
                            ...
                            <td>
                                ...
                                <input type="hidden" name="qty_return_out[]" class="form-control text-right" value="{{$item->qty}}" >
                                <input type="number" name="qty[]" class="form-control text-right" id='qty_return_out_{{$item->id}}' value="{{$item->qty}}" min="1" oninput="this.value = Math.abs(this.value)" onwheel="return false;">
                            </td>
                            <td class="text-left">{{ $item->unit }}</td>
                            <td>
                                {!! Form::textarea('notes[]', $item->reason, ['class' => 'form-control', 'rows' => 2, 'placeholder' => '']) !!}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
