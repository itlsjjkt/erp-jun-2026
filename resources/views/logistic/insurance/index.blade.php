...
@section('content')
    <div class="bgc-white bd bdrs-3 p-20 mB-20">
        <div class="mB-20">
            <a href="{{ route('logistic.insurance.list') }}" class="btn btn-info">
                Buat Asuransi
            </a>
        ...
        <div class="collapse mB-20" id="export" aria-expanded="false">
            <form class="form-horizontal" action="{{ route('logistic.insurance.export')}}" method='POST'>
                {{ csrf_field() }}
    ...
    <script>
        $(document).ready(function() {
            $('#dataTables').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('logistic.insurance.datatables') }}',
                "pageLength": 50,
                columns: [
                    {data: 'doc_no', name: 'insurances.doc_no'},
                    {data: 'company', name: 'insurances.company', searchable: true},
                    {data: 'project', name: 'insurances.project', searchable: false},
                    {data: 'expedition_forwarder', name: 'insurances.expedition_forwarder', searchable: true},
                    {data: 'created_at', name: 'insurances.created_at', searchable: false},
