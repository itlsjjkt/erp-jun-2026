...
@section('breadcrumbs')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Revisi TTB</li>
    </ol>
@endsection
...
@section('content')
        {!! Form::model($ttb, [
                'action' => ['Logistic\TtbController@revision', $ttb->id],
                'method' => 'post', 
        ...
        <div class="bgc-white p-30 bd">
            ...
            <div class="alert alert-warning">
                ...
                - Jika item dihapus maka akan dikembalikan ke Inventory (Reversal)<br>
                - Jika menambah item silahkan hapus terlebih dahalu ITEM sebelumnya <br>
                - QTY item  yang sudah di publish tidak dapat dirubah silahkan melakukan Reversal terlebih dahulu, lalu tambahkan item kembali.
            </div>
            ...
            <table class="table table-bordered"  id="tableTTB">
                <thead>
                    ...
                    <th class="text-center text-uppercase" style="width:150px">QTY TTB</th>
                ...
                <tbody class="item_form">
                    @foreach($ttb_items as $item) 
                        <tr class="product_1">
                            ...
                            <input type="hidden" value="{{ $item->out }}" name="stock_out[]">
                            <input type="hidden" value="{{ $item->code_rack }}" name="code_rack[]">
                            ...
                            <td class="text-center">
                                <a href="{{ route('logistic.ttb.remove_item',['id'=>$item->id,'param' => 'revision']) }}"  title="Hapus Item" data-toggle='tooltip' class='btn btn-outline-danger' id="btn-remove"><span class='ti-back-left icon-lg'></span> </button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="mt-4">
            ...
            <input class="btn btn-danger text-uppercase fsz-sm fw-600" type="submit" name="publish" id="btn-submit-dpm" value="Publish TTB">
        </div>
    {!! Form::close() !!}
@stop
            ...
            $(wrapper).append(
                '<tr class="product[] dpm" level_'+i+'">' +
                    ...
                    '<td style="max-width: 350px">' +
                        ...
                        '<select name="inv_id_new[]" class="form-control productItem product narrow wrap" id="product_'+i+'" required></select>' +
                    '</td>' +
                    '<td class="text-center">' +
                    ...
                    '</td>' +
                    '<td>' +
                        '<input type="number" name="qty_new[]" class="form-control qtyItem  text-right"  min="0.1" required id="qty_ttb_'+i+'" onwheel="return false;">' +
                    '</td>' +
                    '<td class="text-center">' +
                        '<span id="measure_'+i+'"></span>' +
                    '</td>' +
                    '<td>' +
                        '<textarea name="notes_new[]" class="form-control" id="description_'+i+'"></textarea>' +
                    '</td>' +
                    '<td class="text-center">' +
